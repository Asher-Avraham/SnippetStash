...
app.post('/api/snippets', async (req, res) => {
  try {
    ...
    const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Math.random().toString(36).substr(2, 6);
    ...
    if (tags && tags.length > 0) {
      const tagInserts = tags.map(tag => 
        pool.query('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES ($1, $2)', [newSnippet.rows[0].id, tag.id])
      );
...
app.put('/api/snippets/:id', async (req, res) => {
  try {
    ...
    if (tags && tags.length > 0) {
      const tagInserts = tags.map(tag => 
        pool.query('INSERT INTO snippet_tags (snippet_id, tag_id) VALUES ($1, $2)', [id, tag.id])
      );
      await Promise.all(tagInserts);
    }
...
app.delete('/api/favorites/:snippet_id', async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM favorites WHERE snippet_id = $1', [snippet_id]);
    res.status(204).send();
  } catch (err) {
    console.error(err);
    res.status(500).send('Server Error');
